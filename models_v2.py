...
class JEPA(nn.Module):
    ...
    def encode_action(self, actions):
        """
        Encode actions as spatial planes 
        Args:
            actions: Tensor of shape (B, T-1, 2), where each action is (delta x, delta y).
        Returns:
            encoded_actions: Tensor of shape (B, T-1, 1, 65, 65).
        """
        B, T_minus_1, _ = actions.shape
        action_planes = torch.zeros((B, T_minus_1, 1, 65, 65))
        for b in range(B):
    ...
    def forward(self, states, actions):
        ...
        encoded_actions = self.encode_action(actions)  # (B, T-1, 1, 65, 65)
        ...
        for t in range(actions.size(1)):  # T-1 iterations
            action = encoded_actions[:, t]  # (B, 1, 65, 65)
